  ...
  if (lost) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#2A180A' }}>
        ...
        <div className="relative z-10 bg-black/40 backdrop-blur-md rounded-2xl border border-amber-700/15 p-6 max-w-sm w-full text-center">
          ...
          <h2 className="text-lg font-bold text-amber-200 mb-2">Connexion perdue</h2>
          <p className="text-amber-300/50 text-sm mb-5">La connexion avec l'adversaire a ete interrompue.</p>
          <button onClick={handleLeave}
            className="w-full py-3 bg-gradient-to-b from-amber-700 to-amber-800 text-amber-100 font-bold text-sm rounded-xl">
            Retour au menu
          </button>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-3 py-3 sm:py-4 flex flex-col min-h-screen">
        ...
        <header className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <h1 className="text-sm sm:text-base font-bold text-amber-400/70 tracking-widest" style={{ fontFamily: 'Cinzel, Georgia, serif' }}>SONGO</h1>
            <div className="flex items-center justify-center gap-1.5 mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
              <span className="text-[10px] text-green-400/60">En ligne</span>
            </div>
          </div>
          <button onClick={() => setShowRules(true)}
            className="p-1.5 rounded-lg text-amber-400/60 hover:text-amber-300 transition-colors hover:bg-white/5" aria-label="Regles">
        ...
        {gameState.gameOver && isHost && (
          <div className="flex justify-center mb-2">
            <button onClick={handleRestart}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-amber-300 text-xs font-medium bg-amber-800/30 hover:bg-amber-800/50 border border-amber-700/20 transition-all">
